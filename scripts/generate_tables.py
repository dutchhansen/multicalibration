...
from scripts.generate_figures import get_row_with_param_id, get_best_param_id_cf, keep_CF_1_rows
...
def generate_table_tabular_datasets(models, dataset, SAVE_DIR):
    # create dir
...
if __name__ == '__main__':
    save_dir = 'final_figures/tables/'

    # Generate all main tables for tabular datasets (used for Appendix F.2)
    print('Generating tables for tabular datasets (Appendix F.2)')
    ...
    for dataset in ['CreditDefault', 'MEPS', 'BankMarketing', 'HMDA', 'ACSIncome']:
        print(' '*4 + f'Generating table for {dataset}: {models}')
        generate_table_tabular_datasets(models, dataset, save_dir)
